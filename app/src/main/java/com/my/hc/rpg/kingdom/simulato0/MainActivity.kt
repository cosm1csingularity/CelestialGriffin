...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (prefs.getBoolean("activity_exec", false)) {
            Intent(this, Flt::class.java).also { startActivity(it) }
            finish()
        } else {
            val exec = prefs.edit()
    ...
    private fun appLnkDeferred() {
        AppLinkData.fetchDeferredAppLinkData(
            ...
            appLinkData?.let {
                ...
                Hawk.put(DEEP1, firstLink)
                Hawk.put(DEEP2, secondLink)
                Hawk.put(DEEP3, thirdLink)
    ...
    private fun appsdk() {
        val conversionDataListener = object : AppsFlyerConversionListener {
            override fun onConversionDataSuccess(data: Map<String, Any>) {
                ...
                SString = (data["campaign"] as String?)!!
                Log.d("NAMING", "campaign attributed: $SString")

                val tokenizer = StringTokenizer(SString, "_")
                val one = tokenizer.nextToken()
                val two = tokenizer.nextToken()
                val three = tokenizer.nextToken()

                Hawk.put(Constants.CAMP1, one)
                Hawk.put(Constants.CAMP2, two)
                Hawk.put(Constants.CAMP3, three)
                appLnkDeferred()
                invHander()
                finish()
            }
